...
> = {
  'psy-activation': {
    ...
    omaha:
      '#06 Social Contact — phone companionship directly addresses social isolation. KBS Behavior subscale improves with each completed call.',
  ...
  'psy-endowment': {
    ...
    omaha:
      '#13 Caretaking/Parenting — the 40-hour floor reduces caregiver anxiety at the structural level.',
    icd10: 'Z63.6 (Dependent relative needing care at home), Z73.0 (Burnout)',
    metric:
      'First-spend latency (how fast do new members use hour #1?), hoarding rate (do they spend or save?)',
    feeds:
      'psy-activation (the floor enables the first micro-task), mac-ubi (the 40-hour floor IS the Care UBI)',
    targetPage: 'enzyme',
  },
  'psy-proximity': {
    ...
    omaha:
      '#04 Neighborhood Safety, #03 Residence — proximity matching builds neighborhood-level social infrastructure.',
    icd10: 'Z59.89 (Problems related to housing and economic circumstances)',
    metric:
      'Average match distance, completion rate by distance bracket, repeat-pair rate (same helper↔family)',
  ...
  'psy-loss': {
    ...
    omaha:
      '#35 Nutrition, #37 Physical Activity, #25 NMS — every LMN-eligible service has an Omaha problem justification that expires with the LMN.',
    icd10: 'All ICD-10 codes on the LMN become inactive on expiry. Renewal re-activates them.',
    metric:
      'Spend velocity increase in final 30 days, LMN renewal rate (target >90%), churn rate of lapsed LMN members',
    feeds:
      'prod-renewal (the renewal workflow), mac-retention (the financial moat), mac-tax (the 28-36% savings)',
    targetPage: 'enzyme',
  },
  'psy-identity': {
    ...
    omaha:
      "#07 Role Change — the Conductor's identity transforms progressively through skill-aligned tasks and Conductor Certification.",
    icd10:
      'Z56.9 (Unspecified work-related problem) — caregiver role strain is a diagnosable condition.',
    metric:
      'Satisfaction by skill alignment, repeat acceptance by skill type, identity persistence over time',
    feeds:
      'prod-cert (Conductor Certification formalizes the identity shift), psy-streak (aligned tasks sustain streaks)',
    targetPage: 'enzyme',
  },
  'psy-trust': {
    ...
    omaha:
      '#06 Social Contact (trust enables interaction), #15 Abuse prevention (background checks are a safeguard)',
    icd10: 'T74.01XA (Adult neglect prevention), T76.01XA (Suspected abuse — screening trigger)',
    metric:
      'Task acceptance with/without visible badge (A/B test), trust score × match success correlation',
    feeds:
      'prod-timebank (trust enables the first exchange), psy-activation (trust reduces activation energy)',
    targetPage: 'enzyme',
  },
  'psy-streak': {
    ...
    omaha:
      'All intervention categories — streaks measure sustained helping behavior across all Omaha problem areas.',
    icd10: 'Not directly mapped — streaks are a behavioral metric, not a clinical one.',
    metric:
      'Week-over-week retention (streakers vs non-streakers), average streak length, streak × referral correlation',
    feeds:
      'psy-cascade (long-streak members are most likely to refer), mac-turnover (streak culture retains the community)',
    targetPage: 'enzyme',
  },
  'psy-cascade': {
    ...
    omaha:
      '#05 Communication with community resources — the referral network IS the community resource infrastructure.',
    icd10: 'Not directly mapped — virality is a distribution metric.',
    metric:
      'Referral rate after 3rd task, referral conversion, cascade depth (average chain length), viral coefficient (referrals per member)',
    feeds:
      'mac-viral ($0 CAC), mac-ubi (each new member expands the Care UBI pool), prod-timebank (more members = better matching = lower match time)',
    targetPage: 'enzyme',
  },
  'prod-discharge': {
    ...
    omaha:
      'ICD-10 → Omaha crosswalk auto-generates care plan. I50.9 → #27 Circulation. E11.9 → #35 Nutrition. Z87.39 → #25 NMS (fall history). F03.90 → #21 Cognition.',
    icd10: 'All discharge diagnosis codes. The crosswalk handles the translation.',
    metric:
      'Time from discharge to care team assembly (target <48hr), 30-day readmission rate (target <10% vs BCH baseline 15.4%), conversion rate (discharge → founding family)',
    feeds:
      "mac-bch ($364K Y1 revenue), psy-activation (discharge family's first Time Bank task), all prod- nodes (discharge is entry to entire ecosystem)",
    targetPage: 'product',
  },
  'prod-respite': {
    ...
    omaha:
      'Crisis triggers map to multiple problems simultaneously: #13 Caretaking (caregiver hospitalized), #25 NMS (fall), #27 Circulation (cardiac), #21 Cognition (acute confusion).',
    icd10:
      'Emergency diagnosis codes from ER/hospital admission. W19.XXXA (fall), I50.9 (heart failure), R41.0 (disorientation).',
    metric:
      'Response time (<4 hours), fund balance, disbursement frequency, crisis → membership conversion rate',
    feeds:
      'mac-ubi (the safety net that makes Care UBI real), psy-endowment (crisis families get immediate hours), psy-cascade (crisis stories are the most powerful recruitment)',
    targetPage: 'product',
  },
  'prod-timebank': {
    ...
    omaha:
      'Every task type auto-maps to an Omaha intervention. Meals → #28 Digestion-hydration. Rides → #05 Communication. Companionship → #06 Social Contact. This makes community care clinically documentable.',
    icd10:
      'Not directly — Time Bank transactions are coded at the Omaha level, which bridges to ICD-10 via the crosswalk when needed for PACE or CMS reporting.',
    metric:
      'Match latency, hours earned/spent/bought per week (velocity), active members per ZIP code, hours-to-match ratio',
    feeds:
      'All psy- nodes (Time Bank IS the behavioral engine), mac-ubi (Time Bank IS the Care UBI), mac-turnover (Time Bank displaces paid hours, reducing cost)',
    targetPage: 'product',
  },
  'prod-lmn': {
    ...
    omaha:
      '#25 NMS → tai chi (fall prevention). #35 Nutrition → RD counseling. #27 Circulation → cardiac rehab. #37 Physical Activity → fitness programs. #21 Cognition → cognitive stimulation. Each Omaha problem maps to qualifying wellness services.',
    icd10:
      'Z87.39 (falls) → tai chi. E11.9 (diabetes) → nutrition. I50.9 (CHF) → cardiac rehab. F03.90 (dementia) → cognitive programs. The crosswalk drives the Marketplace filtering.',
    metric:
      'LMN activation rate, annual HSA savings per family, marketplace booking volume, provider referral revenue (8-12%)',
    feeds:
      'mac-tax (28-36% savings), mac-retention (LMN is the moat), prod-renewal (annual renewal cycle)',
    targetPage: 'product',
  },
  'prod-renewal': {
    ...
    omaha:
      'KBS reassessment across all LMN-qualifying problems. Knowledge/Behavior/Status scores at 0/6/12 months create longitudinal outcome data.',
    icd10:
      'All LMN ICD-10 codes reviewed and updated during renewal. New diagnoses added, resolved ones removed.',
    metric:
      'Renewal rate (target >90%), lapse-to-churn correlation, KBS improvement between renewals',
    feeds:
      'mac-retention (renewal IS retention), psy-loss (fear of losing tax savings drives renewal), mac-val (longitudinal KBS data feeds actuarial models)',
    targetPage: 'product',
  },
  'prod-wearable': {
    ...
    omaha:
      'HR/HRV → #27 Circulation. SpO2 → #26 Respiration. Sleep → #36 Sleep/Rest. Steps → #37 Physical Activity + #25 NMS.',
    icd10:
      'Anomalies generate clinical alerts mapped to ICD-10: elevated HR → I49.9 (cardiac arrhythmia workup). Low SpO2 → J96.10 (respiratory failure). Sleep decline → G47.00 (insomnia).',
    metric:
      'Anomalies detected, true positive rate, time from anomaly to intervention, hospitalizations predicted',
    feeds:
      "prod-predictive (wearable data IS the predictive model's primary input), mac-pace (wearable data enables PACE margin)",
    targetPage: 'product',
  },
  'prod-predictive': {
    ...
    omaha:
      'UTI prediction: #30 Urinary + #36 Sleep + #21 Cognition + #27 Circulation. Fall prediction: #25 NMS + #37 Physical Activity + #42 Medication. Each prediction maps to Omaha problems for structured intervention.',
    icd10:
      'Predicted diagnoses trigger pre-emptive documentation. "At risk for UTI" → N39.0. "At risk for fall" → Z91.81. Pre-emptive ICD-10 coding enables early intervention billing.',
    metric:
      'Prediction accuracy (sensitivity/specificity), hospitalizations avoided, PACE savings per avoided event ($16,037)',
    feeds:
      'mac-pace (avoided hospitalizations = pure margin), mac-val (predictive model is the IP), prod-wearable (wearable data is primary input)',
    targetPage: 'product',
  },
  'prod-cert': {
    ...
    omaha:
      'Each module → Omaha Intervention Scheme "Teaching/Guidance/Counseling." Safe Transfers → #25 NMS. Medication → #42 Prescribed Medication. Dementia → #21 Cognition. KBS Knowledge subscale improves measurably after each module.',
    icd10:
      'Training justification maps to caregiver burden: Z63.6 (dependent relative), Z73.0 (burnout), Z56.3 (stressful work schedule).',
    metric:
      'Module completion rate, KBS Knowledge improvement pre/post, Conductor confidence score, CII reduction correlated to certification level',
    feeds:
      'psy-identity (certification formalizes identity shift), psy-endowment (5 bonus hours per module), mac-bch (certification revenue stream)',
    targetPage: 'product',
  },
  'mac-bch': {
    ...
    formula:
      '$364K = BCH retainer ($30K) + private pay ($210K) + memberships ($4K) + CII/CRI ($45K) + Conductor Cert ($75K)',
    targetPage: 'ubi',
  },
  'mac-viral': {
    ...
    formula:
      '$0 CAC = BCH discharge + BVSD employer + faith community + wellness referral + Time Bank word-of-mouth',
    targetPage: 'ubi',
  },
  'mac-tax': {
  ...
  'mac-pace': {
    ...
    formula:
      '$1.25M = 40 enrollees × $2,600/mo × 12 - (40 × $1,800/mo × 12) = $800 spread × 40 × 12',
    targetPage: 'ubi',
  },
  'mac-val': {
    ...
    formula:
      '$75-150M+ = proprietary Omaha+wearable dataset + actuarial data + federation licenses + >90% retention',
    targetPage: 'ubi',
  },
  'mac-ubi': {
  ...
  'mac-turnover': {
    ...
    formula:
      '$25-28/hr + equity ($52K/5yr) + health insurance + democratic governance = <15% vs 77% industry',
    targetPage: 'ubi',
  },
};
    ...
    nodes: [
      {
        ...
        desc: "Lowering the friction to say 'yes' to the first micro-task.",
      },
      {
        id: 'psy-endowment',
        name: 'Endowment Effect',
        desc: 'Giving 40 hours upfront so users feel wealthy, not needy.',
      },
      {
        id: 'psy-proximity',
        name: 'Propinquity Effect',
        desc: 'Hyper-local matching within a 2-mile radius.',
      },
      {
        id: 'psy-loss',
        name: 'Loss Aversion',
        desc: 'Graduated credit expiry and fear of losing tax status.',
      },
      {
        id: 'psy-identity',
        name: 'Self-Verification',
        desc: "Aligning tasks with the user's existing identity.",
      },
      {
        id: 'psy-trust',
        name: 'Signaling Theory',
        desc: 'Visible background checks and two-way ratings.',
      },
      {
        id: 'psy-streak',
        name: 'Goal Gradient',
        desc: 'Streak tracking to build continuous habits.',
      },
      {
        id: 'psy-cascade',
        name: 'Viral Loop',
        desc: 'Refer-a-neighbor prompts after the 3rd successful task.',
      },
    ],
  },
  {
    ...
    nodes: [
      {
        ...
        desc: 'HL7 feed integration with BCH Epic.',
      },
      {
        id: 'prod-respite',
        name: 'Respite Emergency Fund',
        desc: '48hr SLA automated dispatch for crisis catch.',
      },
      {
        id: 'prod-timebank',
        name: 'Time Bank Core',
        desc: 'The ledger tracking earned, spent, and bought hours.',
      },
      {
        id: 'prod-lmn',
        name: 'LMN Marketplace',
        desc: 'Automated Letter of Medical Necessity generation.',
      },
      {
        id: 'prod-renewal',
        name: 'Annual Renewal',
        desc: 'Automated telehealth scheduling for LMN upkeep.',
      },
      {
        id: 'prod-wearable',
        name: 'Wearable Integration',
        desc: 'Apple Health API for continuous vital monitoring.',
      },
      {
        id: 'prod-predictive',
        name: 'Predictive Hospitalization',
        desc: 'ML anomaly detection triggering interventions.',
      },
      {
        id: 'prod-cert',
        name: 'Conductor Certification',
        desc: 'LMS integration for family upskilling.',
      },
    ],
  },
  {
    ...
    nodes: [
      {
        id: 'mac-bch',
        name: '$364K Y1 Revenue',
        desc: 'Immediate cash flow from hospital partnerships.',
      },
      {
        id: 'mac-viral',
        name: '$0 Acquisition Cost',
        desc: 'B2B2C distribution eliminates consumer ad spend.',
      },
      {
        id: 'mac-tax',
        name: '28-36% Tax Savings',
        desc: 'The financial moat that prevents churn to agencies.',
      },
      {
        id: 'mac-retention',
        name: 'Infinite Retention',
        desc: 'Leaving the co-op means losing the tax advantage.',
      },
      {
        id: 'mac-pace',
        name: '$1.25M PACE Margin',
        desc: 'Shared savings from avoided readmissions.',
      },
      {
        id: 'mac-val',
        name: '$100M+ Valuation',
        desc: 'Enterprise value driven by proprietary data asset.',
      },
      {
        id: 'mac-ubi',
        name: 'The Care UBI',
        desc: 'A 40-hour guaranteed floor of community care.',
      },
      {
        id: 'mac-turnover',
        name: '<15% Turnover',
        desc: 'Worker-ownership solves the 77% industry churn rate.',
      },
    ],
  },
];
    ...
    >
      ...
      >
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          >
            Master Synthesis Matrix
          </h1>
          <p style={{ fontFamily: fs, fontSize: 18, color: C.t4, lineHeight: 1.6 }}>
            How behavioral psychology unlocks product features, which generate macroeconomic
            outcomes. Select a through-line to see the system in action.
          </p>
        </div>
      </header>
        ...
        >
          {PILLARS.map((pillar, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div
                style={{ marginBottom: 16, borderBottom: `1px solid ${C.dk2}`, paddingBottom: 16 }}
              >
                <h2
                  style={{
                    fontFamily: ff,
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      boxShadow:
                        isActive && activeLine ? `0 8px 24px ${activeData?.color}20` : 'none',
                      cursor: 'pointer',
                      position: 'relative',
                      display: 'flex',
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.transform =
                        isActive && activeLine && !isExpanded ? 'scale(1.02)' : 'scale(1)';
                    }}
                  >
                    <div
                      style={{
                    ...
                    >
                      <h3
                        style={{
                          ...
                          fontSize: 16,
                          fontWeight: 700,
                          color: isActive && activeLine ? activeData?.color : C.w,
                          margin: '0 0 8px',
                        }}
                      >
                        {node.name}
                      ...
                      <span
                        style={{
                          fontSize: 16,
                          color: isActive && activeLine ? activeData?.color : C.t4,
                          opacity: 0.5,
                        }}
                      >
                        {isExpanded ? '−' : '+'}
      ...
      <div style={{ padding: '64px 24px', background: C.dk2, borderTop: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            >
              Clever behavioral economics matched with enzymatic community involvement and real
              economic potential. We leverage everything the community already has to do it all
              better.
            </p>
          </div>
          ...
          >
            {[
              {
                title: '1. The People',
                desc: 'Families are empowered as Conductors, lowering their burden and preventing burnout.',
                icon: '👥',
              },
              {
                title: '2. The Skills',
                desc: 'Neighbors use the Time Bank to provide respite, creating massive, zero-cost liquidity.',
                icon: '⏱️',
              },
              {
                title: '3. The Wellness',
                desc: 'LMNs unlock HSA/FSA funds, driving revenue to local community wellness centers.',
                icon: '🧘',
              },
              {
                title: '4. The Business',
                desc: 'W-2 Co-op professionals provide clinical care with 0% agency extraction and high retention.',
                icon: '💼',
              },
              {
                title: '5. The System',
                desc: 'PACE and hospitals pay for the predictive data and reduced readmissions, funding the whole loop.',
                icon: '🏥',
              },
            ].map((step, i) => (
              <div
              ...
              >
                ...
                <div style={{ fontFamily: fs, fontSize: 13, color: C.t4, lineHeight: 1.5 }}>
                  {step.desc}
                </div>
                {!isMobile && i < 4 && (
                  <div
