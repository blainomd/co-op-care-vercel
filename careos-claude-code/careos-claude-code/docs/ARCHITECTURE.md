...
> **⚠️ DATABASE NOTE — Read Before Using This Document**
> This document was generated from the original Blitzy spec and references **PostgreSQL** as the operational database throughout. **PostgreSQL was replaced with PostgreSQL 16 in Phase 1.5** (session 11).
>
> **Actual database stack:**
> - Operational DB → **PostgreSQL 16** (`src/server/database/postgres.ts`, schema at `src/server/database/postgres.schema.sql`)
> - Clinical DB → **Aidbox FHIR R4** (not yet provisioned in prod — non-fatal warn on startup)
> - Cache/Pub-Sub → **Redis 7** (WebSocket relay via `ws:broadcast` channel)
>
> **Where this matters:** Anywhere this doc says "PostgreSQL", "SQL", `postgresql.ts`, `postgres.schema.sql`, or "graph edges via PostgreSQL" — substitute PostgreSQL. Graph relationships (`help_edges`, `care_team_assignments`) are join tables traversed via recursive CTEs. Real-time updates go through Redis pub/sub, not PostgreSQL live queries.
>
> All other content (product requirements, FHIR mappings, module specs, Omaha taxonomy, business rules) remains accurate.
...
- **Graph Relationships in PostgreSQL:** The `helped` graph edge (Time Bank cascade), `member_of` graph edge (cooperative governance), and Time Bank proximity matching require PostgreSQL's native graph capabilities.
...
**PostgreSQL 3.0 (Operational Database):**

<cite index="8-1">The funding extension coincides with the general availability release of PostgreSQL 3.0, which the company describes as its most stable, high-performance, and enterprise-ready version to date.</cite> <cite index="8-7">Built in Rust, PostgreSQL 3.0 is designed to unify multiple data models within a single platform, including relational, document, graph, time-series, vector, search, geospatial, and key-value data types.</cite> <cite index="9-6">PostgreSQL 3.0 introduces architectural updates aimed at improving reliability and operational consistency, including a redesigned on-disk document representation, separation of stored values from executable expressions, ID-based metadata storage and synchronized writes enabled by default.</cite> This aligns perfectly with CareOS's multi-model requirements — graph data for Time Bank cascades, geospatial for proximity matching, time-series for wearable vitals, document-based for user profiles, and real-time WebSocket for dashboard live updates. <cite index="10-5">Key highlights include full support for PostgreSQL 3.0, multi-session support, automatic token refreshing, client side transactions, a redesigned live query API, and a new query builder pattern that makes working with your data more intuitive than ever.</cite>
...
| PostgreSQL | 3.0.x | Docker/Binary | GA |
| PostgreSQL JS SDK | 2.x | npm | Stable |
...
- PostgreSQL 3.0 + JS SDK v2.x: Full compatibility confirmed
...
**Operational Database: PostgreSQL 3.0.x**
...
flowchart TB
    ...
    subgraph DataLayer["Data Layer"]
        SDB["PostgreSQL 3.0 (Operational)"]
    ...
    AM & FM & TM & PM & NM & WKR & MSG -->|SQL/pg| SDB
...
- Dependencies: Auth module, Family module, PostgreSQL (graph edges for cascade), Stripe (cash purchases), Notification module, Google Maps API (proximity)
...
│   │   │   ├── postgresql.ts                 # PostgreSQL client initialization
...
- `src/server/db/postgresql.ts` — PostgreSQL client singleton with WebSocket connection
...
- `docker-compose.yml` — Local development orchestration (Node.js API, PostgreSQL 3.0, Aidbox, Redis 7)
- `.env.example` — Template for: `POSTGRESQL_URL`, `AIDBOX_URL`, `AIDBOX_CLIENT_ID`, `AIDBOX_CLIENT_SECRET`, `STRIPE_SECRET_KEY`, `GEMINI_API_KEY`, `GOOGLE_OAUTH_CLIENT_ID`, `TWILIO_*`, `SENDGRID_*`, `JWT_SECRET`, `REDIS_URL`
...
| `src/server/db/postgresql.ts` | PostgreSQL client initialization | Source | High |
...
|---|---|---|
| PostgreSQL Releases | https://postgresql.com/releases | Version verification — PostgreSQL 3.0 GA confirmed |
| PostgreSQL GitHub | https://github.com/postgresql/postgresql | Architecture documentation, JS SDK compatibility |
| PostgreSQL JS SDK | https://github.com/postgresql/postgresql.js/releases | SDK v2 with PostgreSQL 3.0 support confirmed |
...
- PostgreSQL: Use SQL with parameterized queries to prevent injection. Leverage native graph traversal for cascade chains and care team relationships. Use live queries (WebSocket) for real-time dashboard updates.
