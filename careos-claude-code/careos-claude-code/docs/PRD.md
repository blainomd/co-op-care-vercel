...
- Deployment: PostgreSQL Cloud (managed) for production
...
- **Missing PostgreSQL Tables:** The schema must include `omaha_assessment`, `conductor_cert`, `referral`, `helped` (graph edge for Time Bank cascade), and `member_of` (graph edge for cooperative governance).
