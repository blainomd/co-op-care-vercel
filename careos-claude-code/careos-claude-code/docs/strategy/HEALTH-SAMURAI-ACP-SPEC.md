...
Our architecture uses SurrealDB as the operational database with an asynchronous outbox pattern to sync clinical data into Aidbox:

```
  SurrealDB (operational store)
       |
       |  DB trigger on write
       v
...
- **SurrealDB** is the primary operational database. All application writes land here first.
...
3. Communication resource is created in SurrealDB and synced to Aidbox
...
| Conflict resolution | Last-write-wins | CareOS/SurrealDB is source of truth |
...
1. Add SurrealDB triggers for the new tables (`advance_directives`, `goals_of_care`, `family_conversations`)
