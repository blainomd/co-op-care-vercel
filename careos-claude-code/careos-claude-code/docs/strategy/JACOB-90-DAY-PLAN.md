...
**Database:** PostgreSQL 16 + PostGIS (NOT SurrealDB). Simpler, battle-tested, cheaper to host. Schema: `src/server/database/schema.sql`

**What changed since the handoff docs:** Blaine filed an LCA (not LLC). Every.io is off — Opolis is ecosystem partner. SurrealDB replaced with PostgreSQL for now. The #1 business priority is getting assessment revenue flowing ASAP. Care Navigators (W-2, $25-28/hr) start in Month 1 doing assessments under Dr. Emdur's supervision.

**Read first:** `JACOB-BACKEND-HANDOFF.md` (v1, 7 endpoints) and `JACOB-BACKEND-HANDOFF-v2.md` (architecture update). Note: those docs contain some SurrealDB references — this plan supersedes them for database choice.
...
**Schema file:** `src/server/database/schema.sql` — 15 tables, 4 relation tables (replacing graph edges), PostGIS for geospatial, `updated_at` triggers, UUID primary keys.

**Key PostgreSQL patterns replacing SurrealDB:**

| SurrealDB Concept | PostgreSQL Equivalent |
|---|---|
...
**Full specs for all 7 are in `JACOB-BACKEND-HANDOFF.md`.** Replace the SurrealQL sketches with standard PostgreSQL — all the same logic applies.

**Example conversions:**

```sql
-- Active neighbors (replaces SurrealQL time::now() - 90d)
...
**4 relation tables (replacing graph edges):** helped, member_of, assigned_to, referred
