...
**Database change (March 10):** We're using **PostgreSQL 16 + PostGIS** instead of SurrealDB for the operational database. Simpler, battle-tested, better tooling ecosystem. Schema: `src/server/database/schema.sql`. The SurrealDB schema file (`surrealdb.schema.surql`) is retained for reference but is not the build target.
...
**Full specs for all 7 are in the original v1 handoff** (`docs/JACOB-BACKEND-HANDOFF.md`). Replace the SurrealQL sketches with standard PostgreSQL — all the same logic applies.
...
- `src/server/database/surrealdb.schema.surql` — retained for reference, not the build target
- `src/shared/types/surrealdb-agentic.types.ts` — concepts apply but specific SurrealDB features don't
...
-- Relation traversal (replaces graph edges)
